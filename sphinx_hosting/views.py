...
from django.views.generic.edit import BaseCreateView, BaseFormView, BaseUpdateView
...
class WildewidgetsMixin(StandardWidgetMixin):  # pylint: disable=abstract-method
    ...
    template_name: str = "sphinx_hosting/base.html"
...
class ProjectListView(  # type: ignore[misc]
    LoginRequiredMixin, WildewidgetsMixin, SphinxHostingMenuMixin, TemplateView
):
    """
    The view we use for the home page of the ``sphinx_hosting`` app.  It
    ...
    def get_breadcrumbs(self) -> SphinxHostingBreadcrumbs:
        return SphinxHostingBreadcrumbs()
...
class ProjectDetailView(  # type: ignore[misc]
    ...
    """
    Handles displaying the details page for a
...
class ProjectCreateView(
    ...
    """
    A POST only view for creating a :py:class:`sphinx_hosting.models.Project`.
    The POST will be submitted from the "Add Project" modal form displayed on
    :py:class:`ProjectListView`.
    """
...
class ProjectUpdateView(  # type: ignore[misc]
    ...
    def get_success_url(self) -> str:
        logger.info(
            "project.update.success project=%s id=%s",
            self.object.machine_name,
            self.object.id,
        )
        return reverse(
...
class ProjectRelatedLinkCreateView(
    LoginRequiredMixin, PermissionRequiredMixin, FormValidMessageMixin, BaseCreateView
):
    ...
    def get_form_kwargs(self) -> Dict[str, str]:
        self.project: Project = get_object_or_404(
            Project, machine_name=self.kwargs["project_slug"]
        )
    ...
    def form_valid(self, form: Form) -> HttpResponse:
        form.instance.project = self.project  # type: ignore[attr-defined]
    ...
    def get_success_url(self) -> str:
        ...
        return reverse(
            "sphinx_hosting:project--update",
            args=[link.project.machine_name],  # type: ignore[attr-defined]
        )


class ProjectRelatedLinkUpdateView(
    LoginRequiredMixin, PermissionRequiredMixin, FormValidMessageMixin, BaseUpdateView
):
    ...
    def get_form_valid_message(self) -> str:
        return (
            f'Updated Related Link "{self.object.title}" for project '
            f'"{self.object.project.machine_name}"'
        )

    def get_success_url(self) -> str:
        logger.info(
            "project.relatedlink.update.success project=%s link_id=%s "
            "link_title=%s link_url=%s",
        ...
        return reverse(
            "sphinx_hosting:project--update", args=[self.object.project.machine_name]
        )
...
class ProjectRelatedLinkDeleteView(
    ...
    def delete(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        response = super().delete(request, *args, **kwargs)
        logger.info(
            "project.relatedlink.delete.success project=%s link_id=%s "
            "link_title=%s link_url=%s",
            self.object.project.machine_name,
            self.object.id,
            self.object.title,
...
class VersionDetailView(  # type: ignore[misc]
    ...
    def get_content(self) -> Widget:
        ...
        """
        title = Block(f"{self.object.project.title} {self.object.version}", tag="span")
        ...
        if self.object.head:
            ...
            if self.object.project.latest_version != self.object:
                layout.add_sidebar_form_button(
                    ...
                    color="lime mb-2",
                    confirm_text=_(
                        "Are you sure you want to make this the latest version "  # noqa: INT001
        ...
        if user.has_perm("sphinxhostingcore.delete_version"):
            layout.add_sidebar_form_button(
                "Delete Version",
                reverse(
                    "sphinx_hosting:version--delete",
                    args=[self.object.project.machine_name, self.object.version],
                ),
                color="outline-secondary",
    ...
    def get_breadcrumbs(self) -> SphinxHostingBreadcrumbs:
        ...
        breadcrumbs.add_breadcrumb(
            self.object.project.title, url=self.object.project.get_absolute_url()
        )
...
class VersionUploadView(
    ...
    def form_valid(self, form: Form):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.messages.error(
                    ...
                    'the "project" field in your Sphinx "conf.py" matches the machine '
                    "name of an existing project here."
                )
        ...
        logger.info(
            ...
            version.project.title,  # type: ignore[attr-defined]
            version.version,
        )
        return super().form_valid(form)
...
class VersionDeleteView(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):
    ...
    def get_queryset(self) -> QuerySet[Version]:
        ...
        :py:class:`sphinx_hosting.models.Version` objects associated with the
        :py:class:`sphinx_hosting.models.Project` identified by our
        ``project_slug`` URLPath kwarg, which is the machine name of our project.
    ...
    def delete(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        response = super().delete(request, *args, **kwargs)
        logger.info(
            "version.delete.success project=%s id=%s",
            self.object.machine_name,
            self.object.pk,
        )
        return response
    ...
    def get_success_url(self) -> str:
        return reverse(
            "sphinx_hosting:project--update", args=[self.kwargs["project_slug"]]
        )
...
class SphinxPageDetailView(  # type: ignore[misc]
    ...
    """
    Handles displaying the actual contents of the :py:class:`SphinxPage`, with
    navigation, table of contents and the actual page content.
    """
    ...
    def get_queryset(self) -> QuerySet[SphinxPage]:
        ...
        if version == "latest":
            ...
            if not version_obj:
                msg = f'Project "{project_slug}" has no versions'
                raise Http404(msg)
            version = version_obj.version  # type: ignore[attr-defined]
        return (
            super()
            .get_queryset()
            .filter(
                version__version=version, version__project__machine_name=project_slug
            )
        )
    ...
    def get_breadcrumbs(self) -> SphinxHostingBreadcrumbs:
        ...
        breadcrumbs.add_breadcrumb(
            self.object.version.version, url=self.object.version.get_absolute_url()
        )
        breadcrumbs.add_breadcrumb(self.object.title)
        return breadcrumbs
