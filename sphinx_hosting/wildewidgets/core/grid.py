...
class Column(Block):
    ...
    Args:
        *blocks: a list of :py:class:`Block` objects to add to this column
    ...
    def __init__(
        ...
        **kwargs
        ...
        for viewport, w in cast(Dict[str, str], self.viewport_widths).items():
            col += f' col-{viewport}-{w}'
        # FIXME: remove this once we fix wildewidgets.Block to work more sanely re: classes
        if self._css_class is None:
            self._css_class = ''
        self._css_class += col
        if self.alignment or self.self_alignment:
            self._css_class += ' d-flex'
        if self.alignment:
            self._css_class += f' justify-content-{self.alignment}'
        if self.self_alignment:
            self._css_class += f' justify-content-{self.self_alignment}'
...
class Row(Block):
    ...
    #: <https://getbootstrap.com/docs/5.2/layout/columns/#vertical-alyi<ignment>`_
