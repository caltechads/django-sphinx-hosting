...
class Command(BaseCommand):
    help = 'Create or update an API-Only user'
    ...
    def handle(self, **options):
        try:
            ...
            if options['update_only'] or options['show_token']:
                print(f"No user with username {options['username']} exists, and you passed either "
                      "--token or --update-only")
                print("Aborting.")
            else:
            ...
            if options['show_token']:
                ...
                print(f'Updated APIUser object for {user.first_name} {user.last_name} ({user.username}): ID={user.id}')
